...
public sealed class MockSeismicDataService : ISeismicDataService
{
    ...
    public SiteEventsPageViewModel? GetSiteEvents(int siteId, string confidence = "All", bool flaggedOnly = false)
    {
        ...
        {
            events = [.. events.Where(e => string.Equals(e.Confidence, confidence, StringComparison.OrdinalIgnoreCase))];
        }

        if (flaggedOnly)
        {
            events = [.. events.Where(e => e.DistanceDeviationFlag || e.HealthWarningFlag)];
        }
